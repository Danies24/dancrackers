...
const statusColors: Record<string, string> = {
  NEW: "bg-maroon-tint text-maroon",
  ...
  return (
    <div className={`rounded-lg border p-3 ${highlight ? "border-red bg-red/5" : "border-border bg-surface"}`}>
      <div className="flex items-start justify-between gap-2">
        <div>
          <Link href={`/admin/orders/${order.id}`} className="font-mono text-sm font-semibold text-maroon">
            {order.order_ref}
          </Link>
      ...
      <div className="mt-2 flex gap-2">
        <a
          ...
          className="flex-1 rounded-md border border-maroon py-1.5 text-center text-xs font-semibold text-maroon"
        >
          Call
        </a>
        <a
